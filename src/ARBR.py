...
for code in index.values():
    ...
    index_zh_a_hist_df = ak.index_zh_a_hist(symbol=code,
                                            period="daily",
                                            start_date=start,
                                            end_date=end)
    ...
    # 计算AR、BR指标
    df['AR'] = ta.SUM(df.HO, timeperiod=tineperiod) / ta.SUM(
        df.OL, timeperiod=tineperiod) * 100
    df['BR'] = ta.SUM(df.HCY, timeperiod=tineperiod) / ta.SUM(
        df.CYL, timeperiod=tineperiod) * 100
